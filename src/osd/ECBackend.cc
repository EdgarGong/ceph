...
void ECBackend::start_read_op(
    ...
    // op封装了读取操作的相关信息，包括优先级、事务 ID、是否执行冗余读取等
    auto &op = tid_to_read_map.emplace(
            ...
            ReadOp(
                    ...
                    for_recovery,
                    _op,
                    // 将 want_to_read 和 to_read 参数移动到 op 对象中
...
void ECBackend::do_read_op(ReadOp &op) {
    ...
    // 遍历每个待读取的对象
//    op.to_read 的类型是 std::map<hobject_t, read_request_t>，
//    它表示每个待读取的对象（hobject_t）以及对应的读取请求（read_request_t）
    for (map<hobject_t, read_request_t>::iterator i = op.to_read.begin();
         i != op.to_read.end();
         ++i) {
//      i->second类型为read_request_t
        bool need_attrs = i->second.want_attrs;

        // 遍历待读取对象的每个需要的分片
        // need类型为std::map<pg_shard_t, std::vector<std::pair<int, int>>>
        for (auto j = i->second.need.begin();
            ...
            // subchunks类型为std::map<hobject_t, std::vector<std::pair<int, int>>>
    ...
    // 遍历每个 pg_shard_t 对应的 ECSubRead 对象，构造消息并添加到待发送列表中
    ...
    // 如果待发送的消息列表不为空，则通过父类的 `send_message_osd_cluster` 方法发送消息
    if (!m.empty()) {
